...
namespace SecurityForms.Classes
{
    class SimpleDes
    {
        ...
        // Initial Permutation table (P 83)
        public  byte[] IP = {
        ...
        // Permuted Choice 2 table (P 98 ) Key-Compression
        public  byte[] PC2 = {
                                            14, 17, 11, 24, 1,  5,
                                            3,  28, 15, 6,  21, 10,
                                            23, 19, 12, 4,  26, 8,
        ...
        // Permutation table (Straight Permutation P 92)
        public  byte[] P = {
        16, 7,  20, 21,
        29, 12, 28, 17,
        1,  15, 23, 26,
         ...
         * in reverse order,  last key is used first and so on. Hence, during
         ...
         */
        public  int[,] subkey = new int[16, 48];
        ...
        public int[] getBlock(string input)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                // string
                String s = Convert.ToString(Convert.ToInt32(input[i]), 16);
                ...
                for (int j = 0; j < 4; j++)
                {
                    inputBits[(4 * i) + j] = Convert.ToInt32(s[j] + "");
                }
            }
            return inputBits;
        }
        ...
        public int[] getKey(string input)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    keyBits[(4 * i) + j] = Convert.ToInt32(s[j] + "");
                }
            }
            return keyBits;
        }
        ...
        public  int[] permute(int[] inputBits, int[] keyBits, bool isDecrypt)
        {
            ...
            Array.ConstrainedCopy(newBits, 0, L, 0, 32);
            Array.ConstrainedCopy(newBits, 32, R, 0, 32);
            ...
            Array.ConstrainedCopy(R, 0, output, 0, 32);
            Array.ConstrainedCopy(L, 0, output, 32, 32);
        ...
        public  int[] KS(int round, int[] key)
        {
            ...
            int[] CnDn = new int[56];
            Array.ConstrainedCopy(C1, 0, CnDn, 0, 28);
            Array.ConstrainedCopy(D1, 0, CnDn, 28, 28);
            ...
            for(int xx = 0; xx < Kn.Length; xx++)
            {
                subkey[round,xx] = Kn[xx];
            }
        ...
        public  int[] fiestel(int[] R, int[] roundKey)
        {
        ...
        public  int[] sBlock(int[] bits)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                // array as input
                int iRow = Convert.ToInt32(sRow, 2);
                int iColumn = Convert.ToInt32(sColumn, 2);
                int x = S[i, (iRow * 16) + iColumn];
                // We get decimal value of the S-box here, but we need to convert
                // it into binary:
                String s = Convert.ToString(Convert.ToInt32(x), 16);
                ...
                for (int j = 0; j < 4; j++)
                {
                    output[(i * 4) + j] = Convert.ToInt32(s[j] + "");
