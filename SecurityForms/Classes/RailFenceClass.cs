...
namespace SecurityForms.Classes
{
    class RailFenceClass
    {
        public string Encrypt(string plaintext, string key)
        {
            ...
            //str= str.Trim();
            str = str.Replace(" ", "");
            char[] plaintextArray = str.ToCharArray();//convert plain text to array of char
            char[,] railarray = new char[2, 100]; //2D array for matrix
        ...
        public string Decrypt(string ciphertext, string key)
        {
            ...
            if (cipherArray.Length % 2 == 0)
                mid = ((cipherArray.Length) / 2) - 1;
            else
                mid = (cipherArray.Length) / 2;
